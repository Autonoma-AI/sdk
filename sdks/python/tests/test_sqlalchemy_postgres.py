"""Integration tests for SQLAlchemy adapter with PostgreSQL via Testcontainers."""
...
@pytest.mark.asyncio
async def test_create_entities_fk_constraint(adapter, db_session):
    """Verify Postgres enforces FK constraints (orphan user should fail)."""
    spec = {
        "User": {"fields": [{"id": "orphan-1", "email": "x@y.com", "organization_id": "nonexistent"}]},
    }
...
@pytest.mark.asyncio
async def test_teardown_scoped(adapter, db_session):
    """Teardown only removes records matching the scope value."""
    spec = {
        "Organization": {"fields": [
            {"id": "org-a", "name": "Org A"},
            {"id": "org-b", "name": "Org B"},
    ...
    assert session.query(Organization).count() == 1
    assert session.query(User).count() == 1
    remaining = session.query(User).first()
